...
class nodePoint(object):
	def __init__(self, uv=None, poly=None, ID=-1):
...
class nodeEdge(object):
	...
	def __init__(self, polygon, point1ID, point2ID):
		self.point1ID = point1ID
		self.point2ID = point2ID
		self.polygon = polygon

	def get_adjacent(self, point):
		if point == self.point1ID:
			return self.point2ID
		return self.point1ID

	def __contains__(self, point):
		return self.point1ID == point or self.point2ID == point

	def __eq__(self, edge):
		if self.point1ID == edge.point1ID and self.point2ID == edge.point2ID:
			return True
		if self.point1ID == edge.point2ID and self.point2ID == edge.point1ID:
			return True
		return False
...
def polyApproxSlice( oPos, cutDir, uDir, vDir, mesh, res ):
	...
	for i in range(faceDiff):
		...
		for j in range(len(vertices)):
			...
			nodePoints.append(nodePoint(uv=nodeUV,
				poly=tempPlane, ID=pad))
		...
		for j in range(len(edges)):
			...
			vtx1 = edgeVertices[0]
			idx = vtx1.find('[')
			id_vtx1 = vtx1[idx + 1: -1]

			vtx2 = edgeVertices[0]
			idx = vtx2.find('[')
			id_vtx2 = vtx2[idx + 1: -1]

			# This is wrong! we have multiple IDs for different vertices
			idx = vtx1.find('.')
			polygon = vtx1[:idx]

			nodeEdges.append(nodeEdge(polygon=polygon, point1ID=vtx1, point2ID=vtx2))
...
def ligLength(origin, insertion, jointCentre, proxMesh, distMesh, resolution ):
	...
	cutAim = pm.angleBetween(euler=True, v1=(0.0, 0.0, 1.0),
...
if __name__ == "__main__":
	print("Hello")
	print(ligLength('proximal', 'distal', 'joint', 'prox', 'dist', 5))
