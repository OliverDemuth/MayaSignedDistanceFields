...
def ligCalc(jointName, meshes, gridSubdiv ,ligSubdiv):
	...
	#	gridSubdiv = number of elements for X-axis (along the ligament), e.g., 20 will result in a cube grid with 21 x 11 x 11 grid points. Note, this scales to O((n+1)*(n/2+1)^2)
	...
	if int(gridSubdiv) % 2 == 0:
		subDiv = int(gridSubdiv) # subdivision is even
	...
	for i in range(len(LigAttributes)):
		...
		sigDistFieldArray = sigDistances.reshape(subDiv + 1, int(subDiv / 2) + 1, int(subDiv / 2) + 1)
...
def sigDistField(origin, insertion, jPos, meshes, subdivision):
	...
	#	meshes = name(s) of the mesh(es) for which the signed distance field is calculated
	#	subdivision = number of elements for X-axis (along the ligament), e.g., 20 will result in a cube grid with 21 x 11 x 11 grid points. Note, this scales to O((n+1)*(n/2+1)^2)
	# ======================================== #
...
def sigDistMesh(mesh, Offset, rotMat, subdivision):
	...
	# create 3D grid where y and z axes are half as long as x

	xElements = np.linspace(0, 1, num = subdivision + 1, endpoint=True, dtype=float)
...
def ligLengthOptMin(sigDistFieldArray, relGridPoints, ligSubdiv):
	...
	boundsList = [(None,None)] * (len(initial_guess))
	boundsList[0] = boundsList[1] = boundsList[len(initial_guess) - 2] = boundsList[len(initial_guess) - 1] = (0,0) # set x and y bounds of origin and insertion to zero
...
# ========== constraints function for optimisation ==========
...
def cons_fun(params, x, ip, rotMat):
	...
	# go through each ligament point (except origin and insertion) and check if any of them intersect with a mesh, i.e., signDist < 0
	...
	for i in range(n):
		...
		if 1e-05 <= tempSignDist < 0: # added minimum value for break condition because if below threshold optimizer will end without considering other ligament points
			break
...
def cost_fun(params, x, ip, rotMat):
	...
	# Input variables:
	#	params = array of Y and Z coordinates of ligament points, i.e., params = [[y_0, z_0],[y_1, z_1], ... ,[y_n-1, z_n-1]]. They are, however, flattened into a single array, i.e., [y0, z0, y1, z1, y2, z2, ... , y_n-1, z_n-1], and therefore need to be extracted.
	#	x = constant X coordinates for ligament points. Passed through args, not part of cost function but constraints function
